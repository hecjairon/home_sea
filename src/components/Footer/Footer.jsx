...
export default function Footer({ data }) {
  const brandParts = data.brand_parts || { first: 'Casa', second: 'Noble' };
    ...
    <footer id="blog" className="bg-navy-dark text-white pt-20 pb-8" aria-label="Pie de página">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          <div>
            <a href="#inicio" className="flex items-center gap-2.5 mb-6" aria-label={data.brand}>
              <HouseIcon />
              <span className="font-display text-xl font-bold">
                {brandParts.first} <span className="text-gold italic">{brandParts.second}</span>
              </span>
            </a>
              ...
              {data.socials.map((social) => (
                ...
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    {SOCIAL_ICONS[social.icon] || SOCIAL_ICONS.instagram}
                  </svg>
                </a>
              ))}
            </div>
